...
(defmacro import-all-statics
  "Import all the static Java methods/fields of a given class into Clojure"
  [cls]
  (let [the-class #^Class (. Class forName (str cls))
        static? (fn [x]
                  (. java.lang.reflect.Modifier 
                    (isStatic (. x (getModifiers)))))
        statics (fn [array]
        ...
        import-field (fn [name]
                       (list 'def (symbol name)
                             (list '. the-class (symbol name))))
        import-method (fn [name]
                        (list 'defmacro (symbol name)
                              '[& args]
                              (list 'list ''. (list 'quote the-class)
                                    (list 'apply 'list
                                          (list 'quote (symbol name))
                                          'args))))]
    `(do
       ~@(map import-field all-fields)
